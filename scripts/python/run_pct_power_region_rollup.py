...
def main():
    ...
    logg_base = "%s/run_pct_power_region_rollup" % (sys_path.Log_dir)
    ...
    if options.start_date and options.end_date:
        ...
        current_time = ptime - (ptime % file_time_res) - 86400
    ...
    proc_script = os.path.join(sys_path.scripts_dir, "pct_power_region_rollup.py")
    site_list = os.path.join(sys_path.site_list_dir, "zip_match_region.csv")
    input_dir = os.path.join(sys_path.Fcst_dir, "netcdf")
    output_dir = sys_path.Total_fcst_dir+"_region"
    
    for dt in date_list:
        # Only process specific hours
        if dt.hour not in [6, 12, 13, 14, 15, 16, 17, 18, 19]:
        ...
        output_file = os.path.join(output_dt_dir, "total_power_regional_forecast.%s.nc" % (dt.strftime("%Y%m%d.%H%M")))
