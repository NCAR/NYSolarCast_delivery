...
def get_run_times( options, cf, logg, times_list):        
    ...
    if options.date:
        ...
        for i in range(start_date_utc, end_date_utc + data_delta_seconds, data_delta_seconds):
            ...
            #print(hr_str)
            #
            # Process only hours for NYS for which we generate the day ahead blended model data 
            #
            if (hr_str in ["06"]):
                #print ("appending ", i )
                times_list.append(i) 
        ...
        # Round down to 6 hours after start of the day
        dtime = dtime - (dtime%86400) + (6*3600)
...
def make_log_option_str(options, cf, model_base_name, date_str, genHour,  logg):
    ...
    # Add file basename
    #
    log_base_name = model_base_name + "_day_ahead." + genHour
...
def  get_model_file(unix_time, cf, options, logg):
    ...
    if options.data_delta == 60:
       ...
       glob_base = "{0}.{1}*".format("wrf_hrrr_blend_hourly", date_hour_min)

       path_components = [cf.Input_day_ahead_fcst_dir_60min,yyyymmdd, glob_base]
    else:
       glob_base = "{0}.{1}*".format("wrf_hrrr_blend", date_hour_min)

       path_components = [cf.Input_day_ahead_fcst_dir_15min,yyyymmdd, glob_base]
...
def  make_model_option_str(options, cf, unix_time, logg):
    ...
    file2 = get_model_file(unix_time - 86400, cf, options, logg)
...
def make_app_cdl_path( cf, model_base_name, logg):
    ...
    cdl_name =  model_base_name + "_day_ahead.cdl"
...
def make_outdir(cf, date_str, logg):
    ...
    #  
    fcst_dir_components = [cf.Fcst_dir, "netcdf", date_str]
...
def pct_power_fcst(options, cf, logg, num_fcsts, model_base_name, unix_time):
    ...
    if model_option_str == "-m ":
       ...
       #
       logg.write_error("No blended model data in past 2 days")
    ...
    # 
    cmd = "pct_power_fcst %s %s %s %s %s %s %s %s %s %s %s" % (log_option_str, 
                                                               debug_option_str,  
                                                               single_fcst_str,
                                                               time_trigger_str,
                                                               model_option_str,
...
def main():
    ...
    # Begin processing
    # 
    logg.write_starting("Beginning power_fcst_day_ahead.py")
