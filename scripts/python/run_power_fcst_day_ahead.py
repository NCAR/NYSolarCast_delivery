...
"""
This scipt runs the power forecast for the day ahead. It is a wrapper for power_fcst.py.
"""
...
def main():
   ...
   if options.begin_date and options.end_date:
       ...
       date_str_option = "-d %s" % pd.to_datetime(current_time, unit='s').strftime("%Y%m%d0600")
   ...
   # 
   logg_base = "%s/power_fcst_day_ahead_%s" % (logDir, model_base) 
   ...
   proc_script = os.path.join(sys_path.scripts_dir, "power_fcst_day_ahead.py")

   if model_lead_resolution == "15":
      cmd = "%s %s -l %s -t 15 -r 15 168 %s " % (proc_script,  date_str_option, logg_base, model_base)
   else:    
      cmd = "%s %s -l %s 42 %s " % (proc_script,  date_str_option, logg_base, model_base)
   #print(cmd)
