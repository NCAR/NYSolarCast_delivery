...
"""
THIS UPLOADS THE FORECAST FILES TO THE SFTP SERVER
"""
...
def main():
    ...
    logg_base = "%s/upload_day_ahead_fcst_to_sftp" % (sys_path.Log_dir)
    ...
    if options.start_date and options.end_date:
        ...
        # Set the hour to always be 6z (only hour the day-ahead is produced)
        current_dt = current_dt.replace(hour=6)
        date_list = [current_dt]
        # Look back over previous day
        LOOKBACK = 86400
    ...
    fcst_dir = os.path.join(sys_path.Data_base_dir, "nwp_blend")
    ...
    sftp_fcst_dir = "/Intake/DS000086-NCAR_NYSERDA_GHI_Forecast/forecasts"
    ...
    for dt in date_list:
        ...
        input_file = os.path.join(fcst_dir, dt.strftime("%Y%m%d"), "wrf_hrrr_blend.%s.nc" % dt.strftime("%Y%m%d.%H%M"))
