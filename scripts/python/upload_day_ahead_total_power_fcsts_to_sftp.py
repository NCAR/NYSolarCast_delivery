...
"""
THIS UPLOADS THE Day Ahead TOTAL POWER FORECAST FILES TO THE SFTP SERVER
"""
...
def main():
    ...
    logg_base = "%s/upload_day_ahead_total_power_fcst_to_sftp" % (sys_path.Log_dir)
    ...
    power_fcst_dir = os.path.join(sys_path.Data_base_dir, "total_power_fcst")
    ...
    for dt in date_list:
        ...
        input_file = os.path.join(power_fcst_dir, dt.strftime("%Y%m%d"),
                                  "total_power_forecast.%s.nc" % dt.strftime("%Y%m%d.%H%M"))
