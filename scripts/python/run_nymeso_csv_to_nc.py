...
"""
THIS RUNS THE CODE TO CONVERT THE OBSERVATIONS FROM CSV TO NC
"""
...
def main():
    ...
    logg_base = "%s/run_nymeso_csv_to_nc" % (sys_path.Log_dir)
    ...
    nymeso_nc_dir = os.path.join(nymeso_base_dir, "15min_nc")

    proc_script = os.path.join(sys_path.scripts_dir, "csv2madis_t.py")
    static_file = os.path.join(sys_path.config_dir, "nymeso_csv_to_nc.json")
    
    for dt in date_list:
        input_file = os.path.join(nymeso_avg_dir, dt.strftime("%Y%m%d"),
                                  'nymeso_15min_obs.%s.csv' % dt.strftime("%Y%m%d.%H%M"))
        if not os.path.exists(input_file):
            logg.write_time("Warning: %s doesn't exist\n" % input_file)
            continue

        output_dt_dir = os.path.join(nymeso_nc_dir, dt.strftime("%Y%m%d"))
        ...
        output_file = os.path.join(output_dt_dir,
                                   "nymeso_15min_obs.%s.nc" % dt.strftime("%Y%m%d.%H%M"))
        cmd = "%s %s %s %s -f station -d '%%Y-%%m-%%d %%H:%%M:%%S' -l %s" % (proc_script, input_file, static_file, output_file, logg_base)
