...
"""
THIS UPLOADS THE 15min CSV OBSERVATION FILES TO THE SFTP SERVER
"""
...
def main():
    """ Main driver function, uploads obs to a hardcoded sftp site
    """
    usage_str = "%prog"
    ...
    logg_base = "%s/upload_obs_to_sftp" % (sys_path.Log_dir)
    ...
    sftp_obs_dir = "/Intake/DS000086-NCAR_NYSERDA_GHI_Forecast/observations"

    if os.path.basename(sftp_obs_dir) not in srv.listdir(os.path.dirname(sftp_obs_dir)):
        logg.write_time("Warning: %s doesn't exist.  Making it\n" % sftp_obs_dir)
        srv.mkdir(sftp_obs_dir)


    for dt in date_list:

        input_file = os.path.join(nymeso_avg_dir, dt.strftime("%Y%m%d"), "nymeso_15min_obs.%s.csv" % dt.strftime("%Y%m%d.%H%M"))
        ...
        dt_subdir = os.path.join(sftp_obs_dir, dt.strftime("%Y%m%d"))
        # If the directory doesn't exist, make it
        if dt.strftime("%Y%m%d") not in srv.listdir(sftp_obs_dir):
