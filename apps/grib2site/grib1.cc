 ...
 */
/* $Id: grib1.cc,v 1.2 2006/12/14 21:17:05 cowie Exp $ */
	...
	)
{
    ...
    if (sp >= endp - 3) {
	logFile->write_time("Error: Can't find `GRIB', skipping\n") ;
	return -2;
    }


    //gribp->hdr = prodp->id;
    gribp->hdr = (char *)emalloc(strlen(prodp->id)+1);
    strcpy(gribp->hdr, prodp->id);
	...
	if ( sp + grib_len > endp ) {
	    logFile->write_time("Error: %s: %d bytes too short\n",
		   gribp->hdr, (sp + grib_len) - endp);
	    return -3;
	}
	if ( strncmp((char*)sp + grib_len - 4, "7777", 4) != 0) {
	    logFile->write_time("Error: %s: no end of product\n",
		   gribp->hdr);
	    return -4;
	}
        sp += 8;
    ...
    sp += g3i(pdsp->len) ;
    if (sp >= endp) {
	logFile->write_time("Error: %s: PDS length too big\n", gribp->hdr) ;
	return -5;
    }
	...
	sp += g3i(gdsp->len);
	if (sp >= endp) {
	    logFile->write_time("Error: %s: GDS length too big\n", gribp->hdr) ;
	    return -6;
	}
    }
	...
	sp += g3i(bmsp->len);
	if (sp >= endp) {
	    logFile->write_time("Error: %s: BMS length too big\n", gribp->hdr) ;
	    return -7;
	}
	/* don't yet have any predefined bit maps */
	if( g2i(bmsp->map_flg) != 0) {
	    logFile->write_time("Error: %s: bad predefined bit map block: %d\n",
    ...
    sp += g3i(bdsp->len);
    if (sp >= endp) {
	logFile->write_time("Error: %s: BDS length too big\n", gribp->hdr) ;
	return -9;
    }

    if( strncmp((char *)sp, "7777", 4) != 0) {
	logFile->write_time("Error: %s: no terminating 7777\n", gribp->hdr) ;
	return -10;
    }
