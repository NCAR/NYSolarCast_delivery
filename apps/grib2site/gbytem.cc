 ...
 */
/* $Id: gbytem.cc,v 1.1 2005/12/15 21:01:26 cowie Exp $ */
 ...
 */
static gbytem*
empty_gbytem(int nbytes)
                      /* number of bytes in byte map */
{
	...
	)
{
    ...
    case GRID_MILLER:
    default:
        logFile->write_time("Error: unrecognized grid type %d\n", g1i(gdsp->type));
        return 0;
    }
...
#define BMS_OVHD	6	/* overhead bytes in bit map section */
	...
	/* number of bits in raw GRIB bitmap */
        ...
        if (g2i(bmsp->map_flg)) {
            logFile->write_time("Error: Predefined bitmap %d unrecognized\n",
                   g2i(bmsp->map_flg));
            return 0;
        }
	if (bmsp_len != npts) {	/* grids for which we replicate pole value */
	    ...
	    default:
		logFile->write_time("Error: bitmap length inconsistency, %d != %d\n",
		       bmsp_len, npts);
		return 0;
	    }
	} else {
